...
S2 <- round( runif(100, atdn.13.tax$S.ls.ci[1], atdn.13.tax$S.ls.ci[2]) )

sim.ls.rad.ci <- mclapply(S2, sim.rad, N = atdn.13.tax$Tot.t, sad = "ls", nb.fit = atdn.13.tax$y.nb2,
                          mc.cores = mc.cores, upper = 1e12)

sim.ls.samp.ci <- mclapply(sim.ls.rad.ci, sim.radsamp, tot.area  = atdn.13.tax$Tot.A,
                           n.plots = atdn.13.tax$N.plots, lmk.fit = atdn.13.tax$lm.k,
                           nb.fit = atdn.13.tax$y.nb2, mc.cores = mc.cores)
...
S3 <- round(runif(100, 5e3, 1.35e4))
sim.tnb.rad <- mclapply(S3, sim.rad, N = atdn.13.tax$Tot.t, sad = "tnb",
                                       nb.fit = atdn.13.tax$y.nb2, mc.cores = mc.cores, upper = 1e20)
...
## Bias at the range of the confidence interval of the observed estimate ##
S4 <- round( runif(250, atdn.13.tax$tovo.S$CIs[4,2], atdn.13.tax$tovo.S$CIs[4,1]) )

sim.tnb.rad.ci <- mclapply(S4, sim.rad, N = atdn.13.tax$Tot.t, sad = "tnb",
                                       nb.fit = atdn.13.tax$y.nb2, mc.cores = mc.cores, upper = 1e20)
sim.tnb.samp.ci <- mclapply(sim.tnb.rad.ci, sim.radsamp, tot.area  = atdn.13.tax$Tot.A,
                           n.plots = atdn.13.tax$N.plots, lmk.fit = atdn.13.tax$lm.k,
                           nb.fit = atdn.13.tax$y.nb2, mc.cores = mc.cores)
