...
f1 <- function(x, ...){
    y <- try(sim.abc(S = x,
                     N = atdn.19$Tot.t,
                     n.plots = atdn.19$N.plots,
                     tot.area= atdn.19$Tot.A,
                     nb.fit = atdn.19$y.nb2,
                     lm.sd.fit = atdn.19$lm.sd,
...
LN.sims <- mclapply(simulated.vals, f1, sad = "lnorm", sdlog = atdn.19$pln.cf[2], lower=1e-20, upper=1e20, mc.cores=10)
##save.image()
...
save(all.sims, sim.ids, sim.y, file="ABC2019a.RData")
