...
is \Sexpr{(sum(dados$N.ind))}, $p=$~\Sexpr{p1}.
Therefore, we have $M=$~\Sexpr{unname(pln.cf[1])-log(p1)}.
...
f.st <- fitdist(dados$N.plots/N.plots, "beta")
cf.st <- coef(f.st)
## likelihood functions
LL.beta <- function(s1, s2){
    -sum ( dbeta(x = dados$N.plots/N.plots, shape1=s1, shape2=s2, log=TRUE) )
}
LL.betat.1 <- function(s1, s2){
    -sum ( sads::dtrunc("beta", x = dados$N.plots/N.plots, trunc=0.1/N.plots, coef = list(shape1=s1, shape2=s2), log=TRUE) )
}
LL.betat.5 <- function(s1, s2){
    -sum ( sads::dtrunc("beta", x = dados$N.plots/N.plots, trunc=0.5/N.plots, coef = list(shape1=s1, shape2=s2), log=TRUE) )
}
LL.betat.9 <- function(s1, s2){
    -sum ( sads::dtrunc("beta", x = dados$N.plots/N.plots, trunc=0.9/N.plots, coef = list(shape1=s1, shape2=s2), log=TRUE) )
}
...
LL2.bbt0 <- function(lmu, lrho){
    -sum(dbetabinom.t0(x = dados$N.plots, size = N.plots, mu = unlogit(lmu), rho = unlogit(lrho), log=TRUE))
}
## fit with mle2
...
\item Using a strict approach of selecting the best SAD model with AIC, negative binomial wins. As noted by \citet{tovo2017}, NB provides a lower total richness estimate compared to Log-series. But now it is still a realistic value (\Sexpr{S.est}). The diagnostic plots show that is so because LS overestimate singletons more than NB. If we go for NB we would check if/how to make the hypothetical SAD for the whole Amazonia from a Negative Binomial, as the codes by Steege et al. do.
