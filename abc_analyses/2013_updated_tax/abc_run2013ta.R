...
f1 <- function(x, ...){
    y <- try(sim.abc(S = x,
                     N = atdn.13.tax$Tot.t,
                     n.plots = atdn.13.tax$N.plots,
                     tot.area= atdn.13.tax$Tot.A,
                     nb.fit = atdn.13.tax$y.nb2,
                     lm.sd.fit = atdn.13.tax$lm.sd,
                     lmk.fit = atdn.13.tax$lm.k,
                     nrep = 1, ...))
...
LN.sims <- mclapply(simulated.vals, f1, sad = "lnorm", sdlog = atdn.13.tax$pln.cf[2], lower=1e-20, upper=1e45, mc.cores=10)
##save.image()
...
save(all.sims, sim.ids, sim.y, file="ABC2013ta.RData")
