...
(S.r.ls <- ulrich(Dec2018$population)[1])
@ 
...
With the we get the log-series RAD for the whole region (Amazon):
...
(S.r.ln <- ulrich(Dec2018$population)[2])
@ 
...
nb.clump <- NB.samp(rad = reg.nb.rad, tot.area = Tot.A, 
                    n.plots = N.plots, 
                    lmean.k = reg.nb.rad.lk, lsd.k = lm.k.sigma, 
                    nrep = 100)
rad.abc <- rad.ls(S=15800, N=Tot.t, alpha = fishers.alpha(Tot.t, 15800))
ls.abc.clump <- NB.samp(rad = rad.abc$y, tot.area = Tot.A, 
                    n.plots = N.plots, 
                    lmean.k = reg.ls.rad.lk, lsd.k = lm.k.sigma, 
                    nrep=100)
@ 
...
lines(rad(ls.abc.clump), col="black")
legend("topright", 
