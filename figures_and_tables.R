...
S.13  <- 
    read.csv("figs_and_tables/estimates_S_table.csv") %>%
    filter(bias.corrected==TRUE&type!="LSE TNB"&sampling=="clump"&type!="TNB"&dataset=="2013") %>%
    mutate(se = (IC.up-IC.low)/4)%>%
    summarise(w.mean = weighted.mean(mean, w=1/se), min=min(IC.low), max=max(IC.up)) %>%    
    unlist()
## 2013 updated
S.13.tax  <- 
    read.csv("figs_and_tables/estimates_S_table.csv") %>%
    filter(bias.corrected==TRUE&type!="LSE TNB"&sampling=="clump"&type!="TNB"&dataset=="2013 updated") %>%
    mutate(se = (IC.up-IC.low)/4)%>%
    summarise(w.mean = weighted.mean(mean, w=1/se), min=min(IC.low), max=max(IC.up)) %>%    
    unlist()
## 2019
...
ls.up.13 <- rad.ls(S = S.13[3], N = atdn.13$Tot.t)$y
## 2013 updated
ls.m.13.tax <- rad.ls(S = S.13.tax[1], N = atdn.13.tax$Tot.t)$y
ls.low.13.tax <- rad.ls(S = S.13.tax[2], N = atdn.13.tax$Tot.t)$y
ls.up.13.tax <- rad.ls(S = S.13.tax[3], N = atdn.13.tax$Tot.t)$y
## 2019
...
f19 <- function(obj, legend=FALSE, ...){
    ...
    lines(pln.op, col =4)
    if(legend)
        legend("topright", c("LS","TNB", "PLN"), pch=1, lty=1, col=c(2:4), bty="n", cex=1.5)
}
...
f20 <- function(obj, legend=FALSE, ...){
    ...
    lines(pln.rp, col =4)
    if(legend)
        legend("topright", c("LS","TNB", "PLN"), lty=1, col=c(2:4), bty="n", cex=1.5)
}

## Octav and rad plots
##png("figs_and_tables/sads_fit_to_samples.png", width=9, height=7, res=150, units="in")
 pdf("figs_and_tables/sads_fit_to_samples.pdf", width=9, height=12)
par(##mar = c(5, 5, 4, 2) + 0.1,
    ##mgp = c(3.25, 1, 0),
    ##oma=c(3,3,0,0),
    ##las = 1,
    bty = "l", 
    cex.main = 1.15,  
    cex.lab = 1.5, font.lab = 2, cex.axis = 1.5,
    lwd = 2,
    mfrow=c(3,2))
f19(atdn.13, cex.lab=1.5, legend=TRUE)
f20(atdn.13, log="xy", cex.lab=1.5, col="grey", axes=FALSE)
axis(1)
axis(2)
mtext("2013", cex=1.25, adj=1)
f19(atdn.13.tax, ylab="", xlab="", cex.lab=1.5)
f20(atdn.13.tax, log="xy", ylab="", xlab="", cex.lab=1.5, col="grey", axes=FALSE)
axis(1)
axis(2)
mtext("2013 updated", cex=1.25, adj=1)
f19(atdn.19, xlab="", ylab = "")
f20(atdn.19, log="xy", xlab="", ylab = "", col="grey", axes=FALSE)
axis(1)
axis(2)
mtext("2019", cex=1.25, adj=1)
dev.off()
