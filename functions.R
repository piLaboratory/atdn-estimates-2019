...
#' Calculates unbiased mean and range species richness estimates from the bias simulations
#'
#' @details This function takes the paired vectors of estimates of
#'     species richness and true species richness from simulated
#'     communities and returns the bias-corrected estimates. These
#'     vectors must be supplied as an object generated by the scripts
#'     in directory 'bias estimation', that simulates richness
#'     estimation under different combinations of regional SADs and
...
##that return range estimates as [upper, lower limit]. 
    ...
    y1 <- with(object$estimates,
               S[S.est.rnd > ci[2] & S.est.rnd < ci[3]])
    y2 <- with(object$estimates,
               S[S.est.clump > ci[2] & S.est.clump < ci[3]])
    if(method=="quantile"){
        lista <- list(y1,y2)
        result <- data.frame(mean = sapply(lista,mean, na.rm =TRUE),
                             IC.low = sapply(lista, quantile, 0.025, na.rm=TRUE),
                             IC.up = sapply(lista, quantile, 0.975, na.rm=TRUE),
                             row.names = c("rnd.samp", "clump.samp"))
    }
    if(method=="lm"){
        x1  <- with(object$estimates,
                    S.est.rnd[S.est.rnd > ci[2] & S.est.rnd < ci[3]])
        x2 <- with(object$estimates,
                   S.est.clump[S.est.clump > ci[2] & S.est.clump < ci[3]])
        lm1 <- lm(y1 ~ x1)
        lm2 <- lm(y2 ~x2)
        plm1 <- predict(lm1, data.frame(x1=ci))
        plm2 <- predict(lm2, data.frame(x2=ci))
        result <- data.frame(mean = c(plm1[1], plm2[1]),
                             IC.low = c(plm1[2], plm2[2]),
                             IC.up = c(plm1[3], plm2[3]),
                             row.names = c("rnd.samp", "clump.samp")
